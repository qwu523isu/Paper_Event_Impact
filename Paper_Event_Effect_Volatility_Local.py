# -*- coding: utf-8 -*-
"""
Created on Sat Jun 14 15:07:51 2025
...
for file_idx, file_path in enumerate(tqdm(window_files, desc="Processing files")):
    # Load data
...
if all_results:
    ...
    output_file = OUTPUT_DIR / f"btc_volatility_analysis_{timestamp}.csv"
...
if results_df is not None and not results_df.empty:
    ...
    # Save plot
    plot_file = OUTPUT_DIR / f"volatility_analysis_plots_{timestamp}.png"
...
if results_df is not None:
    ...
    print(f"Models analyzed: {results_df['model_type'].unique()}")
    print(f"Date range: {results_df['file_date'].min()} to {results_df['file_date'].max()}")
    
    # Model performance
    model_performance = results_df.groupby('model_type')['rmse'].agg(['count', 'mean', 'std'])
    print("\nModel Performance:")
